...
export const browseFoldersLoader = (queryClient: QueryClient) => async ({params}: any) => {
...
export const browserFolderLoaderSettings = (folderId: string) => ({
    queryFn: () => foldersFetcher(sessionStorage.getItem('access-token') as string, folderId, {files: true, folder: true}),
