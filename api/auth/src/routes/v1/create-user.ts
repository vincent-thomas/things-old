...
import { ERROR_TYPE, errorSender, resultSender, sender } from "@things/express-func";
...
createUserV1.post('/', rateLimit, input, async (req, res) => {
  ...
  try {
    ...
    const token = createToken(result.id, ["email", "name"], process.env["AUTH_SIGN_KEY"] as string)
    if (shouldSendCookie) {
      saveToken(res, token)
    }
