...
const createStyle = (key: keyof StyleRule, normal: string, test: Partial<typeof breakPoints>) => {
  const base: Base = {
    [key]: normal,
    "@media": {}
  }
  ...
  Object.entries(test).map((v: [string, unknown], index: number) => {
    base["@media"][`screen and (min-width: ${breakPoints[(v[0] as keyof IBreakPoints)]})`] = {
      [key]: v[1]
    };
    return null;
  })
...
export const title = style([themeClass, {
